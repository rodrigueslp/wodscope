...
import { searchYouTubeVideos, generateCrossFitQueries, type YouTubeVideo } from './tools/search.js'
...
function formatVideoResults(videos: YouTubeVideo[]): string {
  if (videos.length === 0) {
  ...
  return videos.map((video, index) => 
   ...
   Canal: ${video.channel}
   Link: ${video.url}
   ${video.description ? `Descrição: ${video.description.substring(0, 100)}...` : ''}`
  ).join('\n\n')
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'search_videos': {
        const { query, maxResults = 5, language = 'pt' } = args as {
          query: string
          maxResults?: number
          language?: string
        }
      ...
      case 'get_crossfit_tutorial': {
        const { movement, level = 'iniciante' } = args as {
          movement: string
          level?: string
        }
      ...
      case 'get_workout_videos': {
        const { movements } = args as { movements: string[] }
        
        const queries = generateCrossFitQueries(movements)
        const allVideos: { movement: string; videos: YouTubeVideo[] }[] = []
        ...
        const formatted = allVideos.map(({ movement, videos }) => 
          `### ${movement}\n${formatVideoResults(videos)}`
        ).join('\n\n---\n\n')
